-- Exact app name → sketchybar-app-font icon
local exact = {
  ["Live"]                                        = ":ableton:",
  ["Acrobat"]                                     = ":acrobat:",
...
-- Prefix patterns checked in order (app name starts with prefix)
local prefix = {
...
local function icon_map(app)
  if exact[app] then return exact[app] end
  for _, pair in ipairs(prefix) do
    if app:sub(1, #pair[1]) == pair[1] then return pair[2] end
  end
  return ":default:"
end
